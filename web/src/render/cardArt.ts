...
type Rank = Exclude<Value, 'Joker'>;
const VALUE_TO_RANK: Record<Rank, string> = {
...
export function cardKey(card: CardDTO): string {
  if (card.suit === 'Joker' || card.value === 'Joker') return 'joker';
  return `${VALUE_TO_RANK[card.value as Rank]}_${SUIT_TO_FILE[card.suit as Exclude<Suit, 'Joker'>]}`;
}
...
export function allCardKeys(): string[] {
  ...
  for (const value of Object.keys(VALUE_TO_RANK) as Rank[]) {
