...
class ScoreKeeper {
  ...
  loadPreviousScores(roundScores) {
    if (roundScores.length > this.totalRounds) {
      throw new Error(
        `Cannot load ${roundScores.length} rounds, game only has ${this.totalRounds} rounds`
      );
    }
    ...
    roundScores.forEach((scores, index) => {
      ...
      if (scores.length !== this.playerNames.length) {
        throw new Error(
          `Round ${roundNumber} scores must have exactly ${this.playerNames.length} scores`
        );
      }
   ...
   */
  isGameComplete() {
    return this.playerNames.every((name) => this.scores[name].every((score) => score !== null));
  }
  ...
  getNextRoundNumber() {
    for (let round = 1; round <= this.totalRounds; round++) {
      ...
      const hasNullScore = this.playerNames.some((name) => this.scores[name][roundIndex] === null);
      if (hasNullScore) {
        return round;
      }
    }
