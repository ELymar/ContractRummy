...
class StateBasedTestGenerator {
  ...
  static generateStateBasedTest(logFilePath, testOutputPath) {
    ...
    const actionsWithFullState = logLines.filter(log => 
...
${actionsWithFullState.map((log, index) => this.generateStateTest(log, index)).join('\n')}
  ...
  static generateStateReconstruction(fullState, action) {
    ...
    lines.push(`      engine.scoreKeeper = new ScoreKeeper([${reconstructedState.players.map(p => `'${p.name}'`).join(', ')}]);`);
    ...
    reconstructedState.players.forEach((player, index) => {
      ...
      lines.push(`      player${index}.hand.cards = [${player.hand.cards.map(c => `'${c}'`).join(', ')}].map(c => Card.fromString(c));`);
    ...
    // Set deck state
    lines.push(`      engine.state.deck.cards = [${fullState.deck.cards.map(c => `'${c}'`).join(', ')}].map(c => Card.fromString(c));`);
    
    // Set burn pile state  
    lines.push(`      engine.state.burnPile.cards = [${fullState.burnPile.cards.map(c => `'${c}'`).join(', ')}].map(c => Card.fromString(c));`);
    ...
    reconstructedState.downPiles.forEach((pile, index) => {
      ...
      lines.push(`      downPile${index}.cards = [${pile.cards.map(c => `'${c}'`).join(', ')}].map(c => Card.fromString(c));`);
  ...
  static reconstructPreActionState(fullState, action) {
    ...
    if (action.payload && action.payload.handOrder) {
      ...
      player.hand.cards = action.payload.handOrder.map(card => 
    ...
    melds.forEach(meld => {
      meld.cardIndices.forEach(index => {
        if (handOrder[index]) {
          ...
          meldCards.push(`[${card.value === 'Joker' ? '🃏' : this.convertValue(card.value)}${card.suit === 'Joker' ? '' : this.getSuitSymbol(card.suit)}]`);
        }
      });
    });
    return meldCards;
