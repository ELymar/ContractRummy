...
describe('Meld Display Unit Tests', () => {
  describe('DisplayUtils formatting with game engine view objects', () => {
    test('should format game engine downPiles correctly', () => {
      ...
      expect(DisplayUtils.formatMeldSummary(mockDownPiles[0], 0, mockDownPiles[0].owner)).toBe(
        '1. [A♥][A♠][A♣] (dupes) (Player 1)'
      );
      expect(DisplayUtils.formatMeldSummary(mockDownPiles[1], 1, mockDownPiles[1].owner)).toBe(
        '2. [6♠][7♠][8♠][9♠] (sequence) (Player 2)'
      );
    });
    ...
    test('should handle object cards (pre-serialization)', () => {
      ...
      expect(DisplayUtils.formatMeldSummary(pileWithObjects, 0, 'Player 1')).toBe(
        '1. [K♥][K♠][K♣] (set) (Player 1)'
      );
    });
    ...
    test('should handle empty melds gracefully', () => {
      ...
      expect(DisplayUtils.formatMeldSummary(emptyPile, 0, 'TestPlayer')).toBe(
        '1. [Empty Pile] (TestPlayer)'
      );

      // Test null pile
      expect(DisplayUtils.formatPile(null)).toBe('[Empty Pile]');
      expect(DisplayUtils.formatMeldSummary(null, 0, 'TestPlayer')).toBe(
        '1. [Empty Pile] (TestPlayer)'
      );
    });
    ...
    test('should handle various card formats from game engine', () => {
      ...
      expect(DisplayUtils.formatMeldSummary(mixedPile, 0, mixedPile.owner)).toBe(
        '1. [6♠][7♠][8♠] (sequence) (Player 1)'
      );
    });
    ...
    test('should demonstrate the fix for [object Object] bug', () => {
      ...
      expect(DisplayUtils.formatMeldSummary(gameEnginePile, 3, 'Player 1')).toBe(
        '4. [Q♥][Q♦][Q♠] (dupes) (Player 1)'
      );
